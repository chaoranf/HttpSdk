...
/**
 * 重写stack处理，解决返回cookie的问题
 * 
 * @author chaoranf@jumei.com
 * @date 2015-8-13
 */
public class GiveHurlStack extends HurlStack {
	 ...
	 */
	public GiveHurlStack(UrlRewriter urlRewriter) {
	...
	public HttpResponse performRequest(Request<?> request,
		...
		final ProtocolVersion protocolVersion = new ProtocolVersion("HTTP", 1,
				1);
		...
		for (Entry<String, List<String>> header : connection.getHeaderFields()
			...
			if (header.getKey() != null) {
				...
				if (header.getKey().equalsIgnoreCase("set-cookie")) {
					h = new BasicHeader(header.getKey(),
							getCookies(header.getValue()));
				} else {
					h = new BasicHeader(header.getKey(), header.getValue().get(
							0));
				}
	...
	private static String getCookies(List<String> cookies) {
